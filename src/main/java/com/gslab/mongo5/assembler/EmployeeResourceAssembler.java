...
public class EmployeeResourceAssembler implements ResourceAssembler<Employee, Resource<EmployeeView>> {
	...
	public Resource<EmployeeView> toResource(Employee entity) {
		...
		if (entity.getProject() != null) {
			employeeResource.add(linkTo(methodOn(ProjectController.class).getProject(entity.getProject().getId()))
					.withRel("project"));
		}
		if (entity.getRole() != null) {
			employeeResource
					.add(linkTo(methodOn(RoleController.class).getRole(entity.getRole().getId())).withRel("role"));
		}
		if (entity.getManager() != null) {
			employeeResource.add(linkTo(methodOn(EmployeeController.class).getEmployee(entity.getManager().getId()))
					.withRel("manager"));
		}
